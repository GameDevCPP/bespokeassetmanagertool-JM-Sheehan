  ...
  do{
    ...
    //When Create Map button is pressed
      ...
      try{
        ...
        loadingMapName = "map.json";
        outJson.open(loadingMapName);
        //write json object:
...
int DrawMap(ns::MapForJsonStruct ms, tgui::Gui& gui)
{
  //Resets GUI for moving between windows.
       ...
       */
       ...
       saveButton->setPosition("35%","75%");/I know return type is const int*.
       gui.add(saveButton);
       ...
       while (window.isOpen())
       {
           ...
           while (window.pollEvent(event))
           {
               ...
               // Saves current state of map to output file.
                 ...
                 json saveFile = ms;
                 std::ofstream outJson(loadingMapName);
...
/I know return type is const int*.
           }
...
int createMap(tgui::EditBox::Ptr mapWidth, tgui::EditBox::Ptr mapHeight, tgui::EditBox::Ptr tileWidth,
tgui::EditBox::Ptr tileHeight, tgui::EditBox::Ptr mapTexture)
{
  try{
    ...
    std::stringstream ss;
    ss << mapWidth->getText() << ' ' << mapHeight->getText() << ' ' << tileWidth->getText() << ' ' << tileHeight->getText() << ' ' << mapTexture->getText();
    try{
      ss >> width >> height >> createTileWidth >> createTileHeight >> mapName;
    }catch(...){
    ...
    textureForMap = mapName;
    menuS = Map;
    mapS = Create;

    if(!(width > 0 && height > 0 && createTileWidth >0 && createTileHeight > 0 && mapName.size() != 0)){
      std::cout << "Malformed Input" << std::endl;
      restart = true;
    }
  }catch(...){
...
void homeScreen(tgui::Gui& gui)
{
  ...
  editBoxMapName->setPosition({"20%", "10%"});
  ...
  createButton->onPress(&createMap, editMapWidth, editMapHeight, editTileWidth, editTileWidth, editMapTexture);
